...
class ObjDetGraspPipeline():
    def __init__(self) -> None:
        ...
        self.object = rospy.get_param('~target_object')
        self.grasp_link = rospy.get_param('~target_link')
        self.grasp_pose = rospy.get_param('~target_pose')
        self.gripper_arm_offset = 0.22
    ...
    def start(self):
        while not rospy.is_shutdown():
            if self.detected_3dObject is not None and math.isnan(self.detected_3dObject[0]) is False:
                ...
                # go up
                #self.plan_cartesian_srv.call(0.0, 0.0, 0.5)
...
if __name__ == "__main__":
    ...
    pipeline.plan_cartesian_srv.call(0.0, -0.3, 0.0)
    # detected_3dObjectRobotFrame = pipeline.camera_to_robot([0.6336592, -0.358496487, 0.1838227], 'zed2_left_camera_frame', 'base_link')
    # target_pose = Pose(detected_3dObjectRobotFrame, Quaternion(0, 0, 0, 1))
    # pose_in_list = [target_pose.position.x, target_pose.position.y, target_pose.position.z, 
    # target_pose.orientation.x, target_pose.orientation.y, target_pose.orientation.z, target_pose.orientation.w]
    # pipeline.static_tf_broadcast('base_link', "plate_link", pose_in_list)
